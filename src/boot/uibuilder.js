...
export default boot(async ({ app }) => {
    ...
    uibuilder = (function uibuilder() {
         ...
         */
        self.me = function () {
            return 'uibuilderfe.js Version: ' + self.version
        }
        ...
        self.setIOnamespace = function setIOnamespace() {
            ...
            if (ioNamespace === undefined || ioNamespace === '') {
                ...
                //console.log('log', '[uibuilderfe:setIOnamespace] Found via url path: ' + ioNamespace)
            } else {
                //console.log('log', '[uibuilderfe:setIOnamespace] Found via cookie: ' + ioNamespace)
            }

            // Namespace HAS to start with a /
            ioNamespace = '/' + ioNamespace

            //console.log('log', 'uibuilderfe: IO Namespace: ' + ioNamespace)
        ...
        self.checkTimestamp = function checkTimestamp(receivedMsg) {
            if (Object.prototype.hasOwnProperty.call(receivedMsg, 'serverTimestamp')) {
                ...
                if (offset !== self.serverTimeOffset) {
                    ...
                    //console.log('log', 'uibuilderfe:' + self.ioChannels.server + ' (server): Offset changed to: ' + offset)
                }
            }
        }
        ...
        self.ioSetup = function ioSetup() {
            ...
            self.socketOptions = {
                ...
                auth: {
                    homeServerNumber: self.homeServerNumber,
                    jwt: self.jwt,
                    email: self.email
                },
                transportOptions: {
                    ...
                    polling: {
                        extraHeaders: {
                            'x-clientid': `uibuilderfe; ${self.homeServerNumber}`,
                            // Authorization: `Bearer ${self.your_jwt}`
                            //Authorization: 'test', //TODO: Replace with self.jwt variable? // Authorization: `Bearer ${your_jwt}`
                        }
                    },
                },
            }
            //console.log("self.ioNamespace", self.ioNamespace)
            //console.log("self.socketOptions", self.socketOptions)
            ...
            self.socket.on('connect', function ioconnect() {
                console.log('connect self.socket.id', self.socket.id, "self.timerid", self.timerid);
                //console.log('info', '[uibuilderfe:ioSetup] SOCKET CONNECTED - Namespace: ' + self.ioNamespace, ' Server Channel: ', self.ioChannels.server, ' Control Channel: ', self.ioChannels.control)
                //console.log('info', '[uibuilderfe:ioSetup] SOCKET CONNECTED - Namespace: ' + self.ioNamespace, ' Server Channel: ', self.ioChannels.server, ' Control Channel: ', self.ioChannels.control)
            ...
            self.socket.on(self.ioChannels.server, function msgFromServer(receivedMsg) {
                // console.log('info', '[uibuilderfe:ioSetup:on-' + self.ioChannels.server + ' (server)] msg received - Namespace: ' + self.ioNamespace, receivedMsg)
            ...
            self.socket.on(self.ioChannels.control, function ctrlMsgRecvd(receivedCtrlMsg) {
                ...
                switch (receivedCtrlMsg.uibuilderCtrl) {
                    ...
                    case 'shutdown':
                        {
                            //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "shutdown" from server`)
                    ...
                    case 'client connect':
                        {
                            //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "client connect" from server`)
                            //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "client connect" from server`)
                            //console.log('info', `[uibuilderfe:ioSetup:client-connect] Server: Version=${receivedCtrlMsg.version}, Time=${receivedCtrlMsg.serverTimestamp}.`)
                            //console.log('info', `[uibuilderfe:ioSetup:client-connect] Server: Version=${receivedCtrlMsg.version}, Time=${receivedCtrlMsg.serverTimestamp}.`)
                            ...
                            if (self.autoSendReady === true) { // eslint-disable-line no-lonely-if
                                //console.log('info', '[uibuilderfe:ioSetup:client-connect] Auto-sending ready-for-content/replay msg to server')
                                //console.log('info', '[uibuilderfe:ioSetup:client-connect] Auto-sending ready-for-content/replay msg to server')
                    ...
                    // case 'authorised': {
                    //     //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "authorised" from server`, receivedCtrlMsg._auth)
                    ...
                    // case 'not authorised': {
                    //     //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "not authorised" from server`)
                    ...
                    // case 'logged off': {
                    //     //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "logged off" from server`)
                    ...
                    default:
                        {
                            //console.log('info', `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received ${receivedCtrlMsg.uibuilderCtrl} from server`)
            ...
            self.socket.on('disconnect', function onDisconnect(reason) {
                ...
                console.log('info', '[uibuilderfe:ioSetup:disconnect] SOCKET DISCONNECTED - Namespace: ' + self.ioNamespace + ', Reason: ' + reason)
            ...
            self.socket.on('connect_error', function onConnectErr(err) {
                console.log(`[uibuilderfe:ioSetup:connect_error] SOCKET CONNECT ERROR - Namespace: ${self.ioNamespace}, ioPath: ${self.ioPath}, Reason: ${err.message}`, err)
            ...
            self.socket.on('error', function onSocErr(err) {
                console.log('[uibuilderfe:ioSetup:error] SOCKET ERROR from server - MESSAGE: ', err)
            ...
            */
            ...
            /* We really don't need these, just for interest
                ...
                self.socket.on('connect_timeout', function(timeout) {
                    //console.log('log', 'SOCKET CONNECT TIMEOUT - Namespace: ' + ioNamespace + ', Timeout: ' + timeout)
                }) // --- End of socket connect timeout processing ---
                self.socket.on('reconnect', function(attemptNum) {
                    //console.log('log', 'SOCKET RECONNECTED - Namespace: ' + ioNamespace + ', Attempt #: ' + attemptNum)
                }) // --- End of socket reconnect processing ---
                self.socket.on('reconnect_attempt', function(attemptNum) {
                    //console.log('log', 'SOCKET RECONNECT ATTEMPT - Namespace: ' + ioNamespace + ', Attempt #: ' + attemptNum)
                }) // --- End of socket reconnect_attempt processing ---
                self.socket.on('reconnecting', function(attemptNum) {
                    //console.log('log', 'SOCKET RECONNECTING - Namespace: ' + ioNamespace + ', Attempt #: ' + attemptNum)
                }) // --- End of socket reconnecting processing ---
                self.socket.on('reconnect_error', function(err) {
                    //console.log('log', 'SOCKET RECONNECT ERROR - Namespace: ' + ioNamespace + ', Reason: ' + err.message)
                    //console.dir(err)
                }) // --- End of socket reconnect_error processing ---
                self.socket.on('reconnect_failed', function() {
                    //console.log('log', 'SOCKET RECONNECT FAILED - Namespace: ' + ioNamespace)
                }) // --- End of socket reconnect_failed processing ---
                self.socket.on('ping', function() {
                    //console.log('log', 'SOCKET PING')
                }) // --- End of socket ping processing ---
                self.socket.on('pong', function(latency) {
                    //console.log('log', 'SOCKET PONG - Latency: ', latency)
                }) // --- End of socket pong processing ---
             */
        } // ---- End of ioSetup ---- //
        ...
        //     //console.log('info', '[uibuilderfe:markLoggedOut] ', _auth)
        ...
        //     //console.log('log', '[uibuilder:logon] ', self._auth)
        // } // ---- End of logon ---- //
        ...
        //     //console.log('info', '[uibuilderfe:initSecurity] Initialising security')
        ...
        //             //console.log('info', '[uibuilderfe:initSecurity] Set self._auth from localStorage')
        //         } catch (e) {
        ...
        //     } else {
        //         //console.log('info', '[uibuilderfe:updateAuth] Client not authorised or no JWT', _auth)
        ...
        //         //console.log('info', '[uibuilderfe:sendAuth] Returning. ', self._auth)
        //         return self._auth
        ...
        self.send = function send(msgToSend, channel, originator = '') {
            ...
            */
            ...
            //console.log('log', '[uibuilderfe:send] Sending msg - Namespace: ' + self.ioNamespace + ', Channel: ' + channel, msgToSend)
            ...
            //console.log("channel", channel, "msgToSend", msgToSend);
        ...
        self.onChange = function onChange(prop, callback) {
            ...
            //console.log('log', `uibuilderfe:uibuilder:onchange: pushing new callback (event listener) for property: ${prop}`)
        ...
        self.addCSSClass = function addCSSClass(styleDefs) {
            ...
            //console.log('log', '[uibuilderfe:addCSSClass] style: ' + styleDefs)

            let uibStyle = document.getElementById('uibstyle')
            if (uibStyle === null) {
                //console.log('log', '[uibuilderfe:addCSSClass] Adding uibstyle stylesheet')
            ...
            for (let styleDef of styleDefs) {
                //console.log('log', `[uibuilderfe:addCSSClass] Adding style ${styleDef}`)
        ...
        self.newScript = function newScript(script) {
            ...
            //console.log('log', 'uibuilderfe: newCode - script: ' + script)
        ...
        self.newStyle = function newStyle(style) {
            ...
            //console.log('log', 'uibuilderfe: newStyle - style: ' + style)
        ...
        self.showToast = function showToast(msg) {
            ...
            if (self.vueApp && self.vueApp.$bvToast) {
                //console.log('[uibuilder:showToast] Bootstrap-Vue available, using to create a toast')
                self.showToastBVue(content, toastOptions)
            } else {
                //console.log('[uibuilder:showToast] Bootstrap-Vue NOT available, creating a vanilla HTML toast')
        ...
        self.onChange('msg', function onMsgRcvd(msg) {
            ...
            if (componentRef === 'globalNotification') {
                //console.log('[uibuilder:internalOnChange] globalNotification triggered - showToast() ')
            ...
            if (componentRef in vueApp.$refs) {
                ...
                if (msg._uib.options) {
                    ...
                    //console.log('log', '🔢📈 new component instance options received for ref', componentRef, ':', msg._uib.options)

                    Object.keys(msg._uib.options).forEach(function (key) {
                        try {
                            vueApp.$set(vueApp.$refs[componentRef], key, msg._uib.options[key])
                        } catch (e) {
                            //console.log('warn', `[uibuilderfe:internal:onChange] Could not update prop "${key}" for component ref="${componentRef}". Error: ${e.message}`)
                        }
                    })
                }
                ...
                if (msg.payload) {
                    ...
                    //console.log('log', '🔢📈 new component instance value received for ref', componentRef, ':', msg.payload)
        ...
        self.uiReturn = {
            ...
            set: function set(prop, val) {
                ...
                if (excluded.indexOf(prop) !== -1) {
                    ...
                    //console.log('warn', 'uibuilderfe:uibuilder:set: "' + prop + '" is in list of excluded properties, not set')
                    return
                }
            ...
            get: function get(prop) {
                ...
                if (excluded.indexOf(prop) !== -1) {
                    ...
                    //console.log('warn', 'uibuilderfe:uibuilder:get: "' + prop + '" is in list of excluded properties, not get')
                    return
                }
 ...
 **/
            ...
            start: function start(namespace, ioPath, vueApp) {
                if (self.started === true) {
                    //console.log('log', '❌ [uibuilderfe:start] Start function already called. Do not call more than once.')
                    return
                }

                //console.log('log', '[uibuilderfe:start] start() called')
                //console.log('log', '[uibuilderfe:start] start() called')

                // If 1st param is an object ...
                if (toString.call(namespace) === '[object Object]') {
                    //console.log('log', '✅ [uibuilderfe:start] namespace IS an object!')
                    //console.log('log', '✅ [uibuilderfe:start] namespace IS an object!')

                    // Is it the vue instance?
                    if (namespace._isVue === true) {
                        //console.log('log', '✅ [uibuilderfe:start] Vue instance object IS available!')
                        //console.log('log', '✅ [uibuilderfe:start] Vue instance object IS available!')
                        vueApp = namespace
                        namespace = undefined
                    } else {
                        ...
                        if (namespace.userInfo) {
                            self.homeServerNumber = namespace.userInfo.homeServerNumber;
                            self.jwt = namespace.userInfo.jwt;
                            self.email = namespace.userInfo.email;
                        }
                    ...
                    if (!vueApp) { // eslint-disable-line no-lonely-if
                        //console.log('log', '❌ [uibuilderfe:start] Vue app not available!')
                    } else if (toString.call(vueApp) === '[object Object]' && vueApp._isVue === true) {
                        //console.log('log', '✅ [uibuilderfe:start] Vue instance object IS available!')
                    } else {
                        //console.log('log', '❌ [uibuilderfe:start] Vue instance object not available!')
                        vueApp = undefined
                    }
                }

                //console.log('log', '[uibuilderfe:start] Calling params - namespace', namespace, 'ioPath', ioPath, 'vueApp', vueApp)
                //console.log('log', '[uibuilderfe:start] Calling params - namespace', namespace, 'ioPath', ioPath, 'vueApp', vueApp)

                // Save the parameters
                if (namespace !== undefined && namespace !== null) { // Mike
                    self.ioNamespace = namespace
                    //console.log("namespace", namespace)
                    // if (namespace.userInfo !== undefined) {
                    //     self.homeServerNumber = namespace.userInfo.homeServerNumber;
                    //     self.jwt = namespace.userInfo.jwt;
                    //     self.email = namespace.userInfo.email;
                    // }
                ...
                //console.log('log', '[uibuilderfe:start] Final Socket.IO params - namespace', self.ioNamespace, 'ioPath', self.ioPath)
                //console.log('log', '[uibuilderfe:start] Final Socket.IO params - namespace', self.ioNamespace, 'ioPath', self.ioPath, 'clientId', self.clientId)
            ...
            eventSend: function eventSend(domevent, originator = '') {
                ...
                if ((!domevent.constructor.name.endsWith('Event')) || (!domevent.currentTarget)) {
                    //console.log('log', '[uibuilderfe:eventSend] ARGUMENT NOT A DOM EVENT - use data attributes not function arguments to pass data')
                    return
                }
    ...
    app.config.globalProperties.$uibuilder = uibuilder
})
